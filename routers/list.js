import ListRepository from "../infrastructure/ListRepository";
import ListApplicationService from "../Application/ListApplicationService";
...
listRouter.post('/createNewList', authinticate, async (req, res) => {
    const _repo = new ListRepository();
    const service = new ListApplicationService(_repo);

    const {user} = req
    const {listName} = req.body

    service.createList(listName, user.id);
